clear all 
use "D:\Dropbox (managi-lab)\M2フォルダ\Okuyama\Air_pollution\GitHub-code&data\data\raw_data\working_data\descriptive_statistics.dta"
...
cd "D:\Dropbox (managi-lab)\M2フォルダ\Okuyama\Air_pollution\GitHub-code&data\tables"
...
estpost sum `summary'  
esttab using Table_A2.tex, replace cells("count mean(fmt(%5.2f)) sd(fmt(%5.2f)) min(fmt(%5.2f)) max(fmt(%5.2f))") title(Descriptive Statistics) noobs nomtitle nonumber b(3)

estpost sum `summary'   if COVIDwave==1
esttab using Table_A3.tex, replace cells("count mean(fmt(%5.2f)) sd(fmt(%5.2f)) min(fmt(%5.2f)) max(fmt(%5.2f))")  title(Descriptive Statistics During COVID) noobs nomtitle nonumber b(3)

estpost sum `summary'   if firstwave==1
esttab using Table_A4.tex, replace cells("count mean(fmt(%5.2f)) sd(fmt(%5.2f)) min(fmt(%5.2f)) max(fmt(%5.2f))")  title(Descriptive Statistics During First Wave) noobs nomtitle nonumber b(3) 

estpost sum `summary'   if secondwave==1
esttab using Table_A5.tex, replace cells("count mean(fmt(%5.2f)) sd(fmt(%5.2f)) min(fmt(%5.2f)) max(fmt(%5.2f))")  title(Descriptive Statistics During Second Wave) noobs nomtitle nonumber b(3) 
*/